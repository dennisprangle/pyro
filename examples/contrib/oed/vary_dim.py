...
def double_reparam_ace_loss(N, M, D=4, alpha=0.5, sigma=1.0):
    ...
    phi_loss = ((log_w0.squeeze(0) - log_wsum).exp().detach() * lq_theta0_y_detach_y.squeeze(0) - \
                ((log_wl - log_wsum).exp().pow(2).detach() * log_wl).sum(0)).mean(0)

    surrogate_loss = (y_loss + phi_loss).sum()
    eig_estimate = y_loss
